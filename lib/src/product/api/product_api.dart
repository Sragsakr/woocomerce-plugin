...
extension WooProductApi on FlutterWooCommerce {
  ...
  Future<List<WooProduct>> searchProductsByCompleteTerm({
    ...
    int? offset,
    WooSortOrder order = WooSortOrder.desc,
    WooSortOrderBy orderBy =
        WooSortOrderBy.title, // Use title for better search results
    List<int>? parent,
    List<int>? parentExclude,
    String? slug,
    WooFilterStatus status = WooFilterStatus.any,
  ...
  Future<WooProductWithChildrens> getProductWithOptions({
    ...
    try {
      ...
      if (response.statusCode != null &&
          response.statusCode! >= 200 &&
          response.statusCode! < 300) {
        return WooProductWithChildrens.fromData(response.data, product);
      } else {
        throw Exception(
          "API call failed with status code: ${response.statusCode}",
        );
  ...
  Map<String, dynamic> _resolveQueryParametersForGettingProductWithOption(
    List<WooProductFilterWithType> options,
    WooProduct product,
  ) {
    final map = <String, dynamic>{};
